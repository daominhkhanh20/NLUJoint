...
class InputFeatures(object):
    ...
    def __init__(
        self, input_ids, attention_mask, token_type_ids, intent_label_id, slot_labels_ids
    ):
...
class JointProcessor(object):
    ...
    def _create_examples(self, texts, intents, slots, set_type):
        ...
        for i, (text, intent, slot) in enumerate(zip(texts, intents, slots)):
            ...
            examples.append(
                InputExample(
                    guid=guid, words=words, intent_label=intent_label, slot_labels=slot_labels
                )
            )
        return examples
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        ), "Error with slot labels length {} vs {}".format(len(slot_labels_ids), max_seq_len)
